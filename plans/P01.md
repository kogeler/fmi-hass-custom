...
Only tests marked `live` may call external services. They remain excluded from the ordinary offline pytest command. The owner-selected S12 interim invokes them as a separate blocking step after offline tests in `verify.yml`; S15 owns the final required-check and trigger policy.
...
- remain excluded from default offline pytest selection; during S12, invoke them explicitly as a separate blocking step after offline tests in the existing pull-request/default-branch `verify.yml` job;
...
The final repository must provide fast required pull-request checks and separate scheduled compatibility/live checks. The owner-selected S12 interim runs live probes after offline tests in the existing blocking `verify.yml` job; S15 must audit and replace or retain that topology based on the CI requirements below.
...
| S14 | Home Assistant compatibility, dependency, security, and privacy audit | S13 | DONE | 2026-08-01T05:44:29Z | 2026-08-01T06:34:06Z | `OWNER_TO_COMMIT` | `docs/P01/S14.md`; suggested commit: `S14: harden HA compatibility and diagnostics` |
| S15 | Production CI/CD, HACS validation, and release verification | S14 | NOT_STARTED | — | — | — | `docs/P01/S15.md` |
