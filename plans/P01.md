...
| S15 | Production CI/CD, HACS validation, and release verification | S14 | DONE | 2026-08-01T09:25:09Z | 2026-08-01T11:41:29Z | `OWNER_TO_COMMIT` | `docs/P01/S15.md`; suggested commit: `S15: automate required CI and HACS releases` |
...
S15 passed 205 stable offline tests with four live tests deselected and 96% aggregate branch-aware coverage, followed by a separate successful four-test live FMI run. Ruff formatting/linting, Pylint 10.00/10, mypy, hassfest, actionlint, layout/distribution setup, strict version metadata, and the deliberate network-blocking probe all pass. Sixteen release-contract tests cover malformed, unchanged, decreased, mismatched, missing, first-release, synchronization, and exact-notes cases. Nine PR-body tests cover `Unreleased` and dated latest sections, manual-text preservation, managed replacement, malformed inputs, body limits, and the no-head-execution workflow contract. Five CI-policy tests enforce the standard PR/`master`/manual trigger matrix and its narrow event-specific exceptions, required live probes, file-only pip installation, dynamic freeze/recreation ordering, and stable/prerelease failure policy. Current-channel compatibility selected stable HA 2026.7.4/helper 0.13.348 and prerelease HA 2026.8.0b3/helper 0.13.351 without committed compatibility pins; the 140/141-package freezes recreated with clean `pip check` results and each passed the then-current 204-test suite. The current uncommitted tree cannot be addressed by remote HACS validation or GitHub Actions, so HACS, CodeQL, dependency review, the first `master` release run, PR-body updater, tag/Release `1.0.0`, repository topics, and branch rules remain documented owner-run verification after S16 and the completed release branch merge.
