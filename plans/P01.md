...
| Sensors not grouped/named by location | Regression | Restore location-aware grouping and appropriate generated names for new entities while preserving customized/existing IDs through safe migration | S06/S11 | `tests/test_sensor_entities.py`; `tests/test_migrations.py` | `2feec86aacb11b07ff6fbd2997175910599b12a4` | IMPLEMENTED | Fresh location-aware IDs/devices, multiple locations, Unicode/punctuation, unchanged unique IDs, exact legacy migration, customized IDs, collisions, and realistic v0.6.2 multi-entry/suffix histories pass offline. Mark `DONE` after CI. |
...
| Location selection/editing | User-facing maintenance | Add a current Home Assistant reconfigure flow that updates an existing entry's coordinates without forcing users to rebuild automations | S07/S11 | `tests/test_config_flow.py`; `tests/test_migrations.py` | `6044660889856200ad7f8be6a60cdc3f6206969b` | IMPLEMENTED | Validated coordinate changes, duplicate prevention, failure/cancellation rollback, one-entry reload, new-place data, stable config/device/weather/sensor identity, customized IDs, v0.6.2 migration, config-ID collision safety, and realistic post-migration registry preservation pass offline. Mark `DONE` after CI. |
...
| Polar sunrise/sunset | Correctness | Handle incomplete sun events deterministically; add polar-day and polar-night tests | S08 | `tests/test_forecast_characterization.py::test_clear_condition_handles_missing_polar_sun_events`; `tests/test_time_and_missing_data.py` | `1bff418b8641772ae2e29745fcd9db6e872a720a` | IMPLEMENTED | The strict xfail now passes. Missing/both/single sun events use a daytime fallback; aware normal boundaries, unknown codes, numeric strings, and invalid event order are handled deterministically. Mark `DONE` after CI. |
| New issues discovered after 2026-07-31 | Triage | Classify all new open issues; fix any critical/high correctness, availability, migration, security, or compatibility defect in this maintenance effort | S00/S13 | `docs/maintenance/RUNTIME_AUDIT.md` | `c919057c6ddc39283bd88ba75bdf061b55ea9159` | REVIEWED | The canonical fork had no reports or pull requests in the anonymous GitHub API inventory on 2026-08-01; the previously inventoried maintenance descriptions remain unchanged, and recent closed descriptions exposed no unresolved critical/high regression. |
...
| S15 | Production CI/CD, HACS validation, and release verification | S14 | DONE | 2026-08-01T09:25:09Z | 2026-08-01T12:22:26Z | `ff00b62eeb94875a7729e585a00e9c6f94da2027`; `OWNER_TO_COMMIT` | `docs/P01/S15.md`; suggested follow-up: `S15: fix production workflow failures` |
| S16 | Documentation, final verification, and final `AGENTS.md` | S15 | NOT_STARTED | — | — | — | `docs/P01/S16.md` |
...
S15 passed 210 stable offline tests with four live tests deselected and 96% aggregate branch-aware coverage, followed by a separate successful four-test live FMI run. Ruff formatting/linting, Pylint 10.00/10, mypy across 34 source files, hassfest, actionlint, layout/distribution setup, strict version metadata, the exact-exception dependency audit, and the deliberate network-blocking probe all pass. Sixteen release-contract tests include the actual legacy root-manifest first-release layout. Nine PR-body tests cover extraction and privileged-workflow security. Six CI-policy and four dependency-audit tests enforce trigger, install, dynamic compatibility, fork fallback, and vulnerability-exception contracts. Current-channel compatibility selected stable HA 2026.7.4/helper 0.13.348 and prerelease HA 2026.8.0b3/helper 0.13.351; both recreated freezes passed the then-current 204-test suite. PR #1 at `ff00b62` completed every workflow: CI/live, both compatibility jobs, both CodeQL analysis jobs, layout, syntax, and hassfest passed; version exposed the legacy manifest-path bug, dependency review proved GitHub's fork limitation, CodeQL found three coordinate-logging alerts in an obsolete debug script, and HACS required Issues/topics. All code failures have local regression fixes. Issues and topics were configured through the owner-authorized `gh` session, and the failed HACS job passed on rerun. The local follow-up awaits the owner commit/push needed to confirm the remaining corrected checks remotely; publication, branch rules, and final S16 verification remain pending.
