...
3. Add comprehensive automated tests for all critical behavior:
   ...
   - marker-isolated live tests that call FMI's real Open Data API and verify the data exposed to Home Assistant.
4. Reproduce and resolve all currently known maintenance reports listed in this plan, including the two enhancements necessary for a complete maintenance release.
5. Implement the documented polar-day/polar-night behavior and add regression tests.
...
7. Build reliable CI for validation, tests, coverage, dependency/security checks, Home Assistant compatibility, HACS validation, and required live FMI checks on pull requests and `master` pushes.
...
Every structural change must be explained in the session handoff with the reported behavior, failing test, current API requirement, or CI rule that made it necessary.
...
- the exact reported behavior and attempted solution are documented;
...
Only tests marked `live` may call external services. They remain excluded from the ordinary offline pytest command, then S15 selects them in a separate required CI step after offline coverage for every pull request and eligible `master` release push. They use no secrets, retain bounded requests/timeouts, and are release-blocking by explicit owner decision.
...
- use prereleases only in a separate compatibility job that runs for pull requests, `master`
  pushes, and manual dispatch; keep that moving prerelease signal informational rather than part
...
- Put the pip bootstrap tool in its own frozen requirements file and require every automated `pip install` to consume requirements/constraint files; never embed package names or versions in workflow, Makefile, container, or CI-helper install commands.
- Treat deliberately moving compatibility environments separately from the committed supported lock: keep their direct selectors unpinned in dedicated input files, resolve latest stable/prerelease independently, capture the complete result with `pip freeze`, recreate a clean environment from that runtime freeze, and only then test it. Do not describe those moving inputs as reproducible locks.
...
- Treat this plan as a living project record. As soon as verified evidence shows an inaccuracy, a mismatch with the real repository/code, or a necessary scope expansion, update the affected plan text, tracker, work matrix, or session specification immediately. Do not defer a known correction until session end.
...
- seven known maintenance reports and proposals, listed below by descriptive name.

S00 must refresh this snapshot against:

- the repository's actual default branch and commit history;
- current open and recently closed maintenance reports;
...
- Local `master` and `origin/master` for `kogeler/fmi-hass-custom` resolved to the baseline commit `77a265cf910050aa6e2e37a4eb57d951bddc2910` before maintenance work began.
- The verified maintenance inventory contains Lightning maximum age, Wind gust unavailable, Sensors not grouped by location, Incorrect daily forecast, Location selection/editing, Forecast failure disables observations, and Polar sunrise/sunset; the closure matrix below remains current.
...
S15 replaced those hand-maintained compatibility selections with current-channel resolution. Its 2026-08-01 verification selected stable HA 2026.7.4/helper 0.13.348 and prerelease HA 2026.8.0b3/helper 0.13.351 without committed compatibility version pins; both generated full freezes recreated cleanly and passed 204 offline tests.

---

## 6. Known maintenance-work closure matrix
...
|---|---|---|---|---|---|---|---|
| Lightning maximum age | Enhancement/reliability | Add a user-configurable maximum strike age; discard older items safely; preserve a documented backward-compatible default | S09 | `tests/test_auxiliary_payloads.py`; `tests/test_config_flow.py`; `tests/test_availability.py` | `033c574507734941aac7cc30a27ce4e8954d0c0c` | DONE | A translated 1-1440 minute option defaults lazily to the prior 24-hour window. Inclusive boundary, aware timestamps, malformed data, query range, option validation/default, source isolation, and recovery passed offline and in required PR CI. |
| Wind gust unavailable | Regression | Restore a correctly typed and available wind-gust sensor when FMI provides valid gust data; do not invent a value when data is absent | S06 | `tests/test_sensor_entities.py`; `tests/test_fmi_contract.py`; `tests/test_forecast_characterization.py` | `2feec86aacb11b07ff6fbd2997175910599b12a4` | DONE | Forecast/current requests add `HourlyMaximumGust` without a second call and normalize it into the client model; observations retain `WindGust`. Valid, fallback, missing, NaN, infinity, zero, conversion, and recovery passed required PR CI. |
| Sensors not grouped/named by location | Regression | Restore location-aware grouping and appropriate generated names for new entities while preserving customized/existing IDs through safe migration | S06/S11 | `tests/test_sensor_entities.py`; `tests/test_migrations.py` | `2feec86aacb11b07ff6fbd2997175910599b12a4`; `45961ff83fb917e602640295ff1762560b95814b` | DONE | Fresh location-aware IDs/devices, multiple locations, Unicode/punctuation, unchanged unique IDs, exact legacy migration, customized IDs, collisions, and realistic v0.6.2 multi-entry/suffix histories passed required PR CI. |
| Incorrect daily forecast | Correctness | Replace first-sample-of-day behavior with documented, deterministic daily aggregation, including summing hourly precipitation amounts | S05 | `tests/test_forecast_characterization.py` | `796d41659c443479dbfeaeef3ed4ca909aca7a7f` | DONE | The former strict xfail is a normal regression. Hourly/daily schema, local-date/DST grouping, precipitation, temperature, condition, wind, averages, missing values, partial days, ordering, duplicates, and HA subscriber exposure passed required PR CI. |
| Location selection/editing | User-facing maintenance | Add a current Home Assistant reconfigure flow that updates an existing entry's coordinates without forcing users to rebuild automations | S07/S11 | `tests/test_config_flow.py`; `tests/test_migrations.py` | `6044660889856200ad7f8be6a60cdc3f6206969b`; `45961ff83fb917e602640295ff1762560b95814b` | DONE | Validated coordinate changes, duplicate prevention, rollback, one-entry reload, stable identities, customized IDs, and realistic v0.6.2 migration passed required PR CI. |
| Forecast failure disables observations | Availability | Degrade gracefully: observations/current conditions remain usable when forecast WFS fails; forecast entities/data show unavailable/empty appropriately and recover automatically | S04 | `tests/test_availability.py` | `ad0c997ea70f2316195808347501de4b4e8f58ea` | DONE | Setup/retry, fallback, stale-data, recovery, lifecycle, listener, and log-transition behavior passed required PR CI. |
| Polar sunrise/sunset | Correctness | Handle incomplete sun events deterministically; add polar-day and polar-night tests | S08 | `tests/test_forecast_characterization.py::test_clear_condition_handles_missing_polar_sun_events`; `tests/test_time_and_missing_data.py` | `1bff418b8641772ae2e29745fcd9db6e872a720a` | DONE | The former strict xfail is a normal regression. Missing/both/single sun events, aware boundaries, unknown codes, numeric strings, and invalid event order passed required PR CI. |
| New issues discovered after 2026-07-31 | Triage | Classify all new open issues; fix any critical/high correctness, availability, migration, security, or compatibility defect in this maintenance effort | S00/S13 | `docs/maintenance/RUNTIME.md` | `c919057c6ddc39283bd88ba75bdf061b55ea9159` | DONE | The canonical fork had no new reports requiring another closure row, and the evidence-based runtime/security audit found no unresolved integration-owned critical/high defect. |

### Closure policy

- Bugs and regressions in the matrix must be fixed.
- lightning maximum-age enhancement and location reconfiguration requirement are included as required deliverables, not optional backlog items.
...
S13 verified this list on 2026-08-01. The only reproduced high runtime defect was an incomplete external-source exception boundary: request-library transport failures skipped current place fallback, and forecast parser failures disabled the whole primary coordinator. The focused fix preserves source isolation for a finite set of expected transport/parser/shape failures. The `logging.basicConfig` hypothesis was also confirmed as a medium process-wide side effect and removed. Earlier sessions' regressions remained closed; coordinator lifecycle, request cadence, event-loop isolation, missing-data/time behavior, and Home Assistant warnings produced no additional critical/high finding. Debug/upstream-parser coordinate disclosure remains explicitly assigned to the S14 privacy audit, and the optional lightning polar-coordinate bounding-box edge is documented as low severity. The resulting current contract is in `docs/maintenance/RUNTIME.md`; historical evidence remains in the session handoffs.
...
Ordinary offline pytest targets must support isolated `pytest-xdist` worker processes through `PYTEST_WORKERS`. Keep the measured-faster worker count as the default, re-benchmark as the suite grows, and do not describe parallelism as an optimization when startup/import overhead makes it slower. Do not run Home Assistant tests in shared threads: their per-test event loops and process-global integration state require process isolation. Live FMI tests and the deliberate network-block probe remain sequential.
...
- remain excluded from default offline pytest selection and run in a distinct required `ci.yml` step after the offline suite for pull requests and eligible `master` release pushes after S15;
...
- never use `continue-on-error`; the S12 interim is blocking, while S15 must make the final decision about trigger/job separation and upstream-outage handling;
...
- every fixed maintenance report: at least one explicit regression test whose name or comment describes the reported behavior;
...
S06 verified its checkpoint on 2026-07-31: `sensor.py` and the new FMI adapter are clean, and the seven remaining repository errors are exactly the three S08-owned `utils.py` errors plus the four S09-owned `__init__.py` errors. S07 preserved that exact seven-error production checkpoint; its config-flow, migration, and tests are mypy-clean. S08 resolved all three `utils.py` errors without ignores; the repository now reports exactly the four S09-owned `__init__.py` parser errors.
...
The final repository must provide required pull-request/release checks, including bounded live FMI probes after offline tests, plus event-driven and manually triggerable compatibility checks. No GitHub Actions workflow may use a `schedule` trigger.
...
- managed pull-request body synchronization from the source branch's latest changelog section;
- verification that ordinary tests made no external network calls.
- current stable Home Assistant compatibility resolution and recreated-suite execution.

### Compatibility and manual checks

- freshly resolve, freeze, recreate, and test the latest Home Assistant stable plus current compatible dependencies;
- freshly resolve, freeze, recreate, and test the current Home Assistant beta/prerelease as an early warning, clearly separated from the release gate;
- run both compatibility jobs for pull requests and `master` pushes as well as by manual dispatch;
- allow safe read-only branch checks such as CI, validation, CodeQL, and version comparison to be
  dispatched manually before a pull request exists;
- live FMI end-to-end tests;
- optional check for newly opened repository reports, FMI changelog changes, and dependency releases, if it can be implemented without credentials or noisy automation.
...
- do not use `pull_request_target` to check out or execute untrusted fork code; a narrowly scoped metadata-only workflow may read a source-branch changelog as inert data and update the PR when it has no secrets, uses only the minimum `pull-requests: write` permission, and runs only trusted base-branch helpers;
...
Do not push, open pull requests, publish releases, close reports, or create tags unless the repository owner explicitly requests it.
...
1. Inspect the repository, default branch, tags, recent commits, open reports, recently closed reports likely related to regressions, and proposed changes.
...
6. Create `plans/P01/BASELINE.md` containing:
   ...
   - refreshed maintenance-report inventory;
...
10. Update the baseline and known-work tables in this plan if repository evidence changed.
...
- Tracker and work matrix reflect current reality.
...
- Do not move files.
- Do not implement reported fixes.
...
6. Represent known defects using strict `xfail` tests or clearly failing branch-specific tests tied to descriptive behavior names. A strict `xfail` must be converted to a normal passing test in the owning fix session.
...
- Full offline suite passes with only explicitly documented strict `xfail` cases for known defects.
...
13. If newest stable cannot be used:
    ...
    - search for or reference a maintenance report;
...
- All offline characterization/contract tests pass except report-specific `xfail` tests assigned to later sessions.
...
## Session S04 — Coordinator/setup resilience and forecast-outage availability regression
...
1. Reproduce forecast-outage availability regression using offline fixtures/exceptions.
...
10. Add a failing offline reproduction for forecast-outage availability regression, then make it a normal passing regression test. S02 did not add a forecast-outage availability regression strict `xfail`; do not claim one exists.
...
- forecast-outage availability regression behavior is fixed and tested.
...
10. Convert incorrect daily forecast regression's strict `xfail` into passing tests.
...
- incorrect daily forecast regression fixed.
...
## Session S06 — Sensor/entity correctness and wind-gust and location-grouping regressions
...
1. Reproduce wind-gust availability regression against the upgraded FMI client and identify the authoritative gust field(s), units, and missing-data behavior. Do not mistake a parser/model unit test for the reported Home Assistant entity failure.
2. Restore wind-gust state when valid data exists. Client 1.0.0 still sends forecast `WindGust`, which the current FMI producer returns as NaN, while the producer exposes `HourlyMaximumGust`; add a narrow offline-tested client adapter that requests and maps the forecast field without adding a second FMI request. Preserve observation `WindGust` semantics and define explicit finite-value precedence.
3. When gust data is absent, represent absence correctly; do not substitute ordinary wind speed or a fabricated zero.
4. Reproduce sensor location-grouping regression on a fresh-entry fixture.
...
9. No full wind-gust and location-grouping regressions strict `xfail` was actually created in S02. Add failing Home Assistant regressions before the production change, then leave them as normal passing tests. Complete registry migration coverage in S11.
...
- wind-gust availability regression fixed.
- sensor location-grouping regression fixed for new entries and migration design implemented sufficiently for S11 verification.
...
7. Decide whether to offer a “use Home zone coordinates” mode:
   - safe coordinate editing is mandatory and is sufficient to resolve location reconfiguration requirement;
   ...
   - verified S07 decision: do not implement dynamic tracking in this maintenance effort; manual reconfigure resolves location reconfiguration requirement without adding another subscription/migration lifecycle. Document this limitation.
...
10. Convert location reconfiguration requirement's regression/acceptance tests to passing.
...
- location reconfiguration requirement is resolved by documented behavior.
- Full offline suite passes.
...
1. Reproduce the Polar sunrise/sunset behavior from its descriptive requirement and verify the intended fallback against deterministic tests.
...
6. Preserve and extend S05/S06 finite-value handling: S06 already made sensor `None`, NaN, infinity, zero, and fallback extraction safe. Complete robustness for numeric strings if the dependency can emit them and for unexpected/missing fields across non-sensor paths. Reject invalid values rather than propagating exceptions or false data.
...
10. Convert the Polar sunrise/sunset regression tests to normal passing tests.
...
Verified S08 decisions and audit outcome (2026-07-31):

- use a daytime `sunny` fallback whenever either sun event is missing, naive, or inconsistent;
- use Home Assistant's configured timezone and complete local-date equality for best-condition selection; S05's forecast grouping already uses complete local dates and is extended with leap-day coverage;
- normalize finite numeric strings at FMI value boundaries, report unknown symbols/missing fields as unavailable, and ignore forecast samples without aware timestamps;
- remove direct `python-dateutil` after replacing its final integration imports with Home Assistant time helpers; regenerate the complete freeze, where it remains test-transitive through the Home Assistant pytest helper;
- retain S09 ownership of the confirmed naive lightning/sea-level request timestamps and lightning string timestamp contract rather than partially changing optional-source behavior in S08. See `docs/maintenance/TIME_AND_MISSING_DATA.md`.
...
- The Polar sunrise/sunset failure mode is fixed with automated tests.
...
1. Implement lightning maximum-age enhancement with a translated user option for maximum lightning-strike age.
...
10. Convert lightning maximum-age enhancement acceptance tests to passing.
...
Verified S09 decisions and audit outcome (2026-07-31):

- configure lightning maximum age in minutes from 1 through 1440, defaulting to the prior 24-hour window; retain strikes exactly at the boundary and apply the default lazily without a config-entry version bump;
- use Home Assistant's shared aiohttp session with 2-second connect, 3-second read, and 5-second total timeouts, a 2 MiB payload bound, explicit HTTP client/server/transport/timeout/parse classification, and no per-update retries;
- retain only aware UTC lightning and sea-level timestamps, reject unsafe parallel-array mismatches, discard malformed individual lightning rows, and clear stale optional data without disabling current weather, observations, or forecasts;
- perform XML parsing and reverse geocoding in Home Assistant's executor; cache address results/failures, allow at most one new address lookup per update and one process-wide lookup per 15 seconds, preserve raw coordinates on failure/rate limiting, and expose OpenStreetMap attribution. See `docs/maintenance/OPTIONAL_SOURCES.md`.
...
- lightning maximum-age enhancement implemented and documented.
...
4. Test current weather, hourly forecast, daily forecast, normal sensors, the observation-backed weather entity, lightning, and sea-level entities through Home Assistant state/entity APIs. The integration has never exposed a separate observation-sensor set; adding one is outside this maintenance plan.
...
S10 verified 100% line/branch coverage for config flow, 95.48% line and 90% branch coverage for setup/coordinators, 96.68% line and 90% branch coverage for weather, and 96.58% line and 97.72% branch coverage for sensors. The pre-existing non-S10 adapter and utility gaps are recorded for S13 rather than being obscured by the 95% aggregate result.

### Exit criteria

- All config/lifecycle behavior is protected by Home Assistant-level tests.
- S10-owned config-flow/setup/coordinator/entity coverage targets are met; registry migration branches remain explicitly assigned to S11, and the separately recorded adapter/utility gaps remain assigned to S13 before final verification.
- Full offline suite passes.
...
4. Verify collision-safe fallback behavior when a target config unique ID or entity ID already exists; preserve both records without partial overwrites.
...
9. Complete sensor location-grouping regression migration acceptance tests.
...
12. Complete the conservative S06 sensor-ID migration evidence for ambiguous automatically suffixed legacy defaults and multi-entry histories. Preserve the S06 rules that customized IDs and occupied targets are never overwritten.
...
S11 verified the eight-test migration suite twice consecutively against Home Assistant 2026.7.4, then passed the 154-test offline suite. Config flow remains at 100% line/branch coverage and setup/coordinators at 95.86% statement and 91.67% branch coverage. Registry snapshots retain every pre-existing internal and unique ID; the only entity-ID changes are the documented in-place renames of unambiguous exact defaults.
...
- sensor location-grouping regression and location reconfiguration requirement migration portions are complete.
- Full offline suite passes and S11-owned config/registry/setup coverage targets are met; the adapter/utility gaps already assigned to S13 remain outside this session.
...
3. Create `docs/maintenance/LIVE_TESTS.md` containing:
   ...
   - license/attribution;
   - privacy rules.
4. Add marker-isolated live tests at two levels:
...
8. As an owner-selected interim, add the live selection as a separate blocking step after offline tests in the existing `verify.yml` job. S15 will audit its security and decide the final triggers/job separation.
...
S12 verified four selected live tests against the current FMI WFS service: southern and northern forecast contracts, current Kumpula station data, Home Assistant current/observation entities and units, hourly/daily forecast services, and local-day precipitation sums all passed. The post-live offline suite passed 155 tests with all four live tests deselected; a networkless targeted run passed the malformed-contract diagnostic and deselected the live module without opening a socket. The documented nominal/worst-case request budgets are 6/10 per complete run versus FMI's published 20,000/day and 600/5-minute limits.
...
- Live tests are marker-isolated from ordinary offline commands and rate-limit-conscious; the interim CI invokes them explicitly.
...
5. Classify findings and use them to establish the current contract in `docs/maintenance/RUNTIME.md`:
   - critical;
   - high;
   - medium;
   - low;
...
10. Close the verified S10 residual coverage gaps in `fmi_client.py` (93.93% line/64.28% branch) and `utils.py` (90.41% line/92.85% branch) with behavioral tests, or document why a genuinely unreachable path is excluded. Do not rely on the aggregate percentage to mask a module below the final Section 8.5 target.
...
S13 reproduced three pre-fix failures: import-time global logging configuration, a request transport failure bypassing place fallback/stale clearing, and malformed forecast XML disabling valid current weather. The focused fixes and 10 added offline cases passed in the final 165-test suite with 4 live tests deselected and 96% aggregate branch-aware coverage. `fmi_client.py` and `utils.py` now have 100% line/branch coverage; setup/coordinators remain at 95.13% lines and 91.67% branches. Home Assistant `protect_loop` guards the synchronous adapter test, the complete suite passes with warnings treated as errors, the deliberate network probe remains blocked, and all four public live FMI probes passed after the request/error-boundary change. No critical or unresolved high finding remains; the durable runtime contract is in `docs/maintenance/RUNTIME.md`.
...
10. Create `docs/maintenance/COMPATIBILITY_SECURITY.md` with the current support, security, privacy, and accepted-risk contract; keep audit history in the handoff.
...
S14 passed 173 stable offline tests with four live tests deselected, warnings treated as errors, and 96% aggregate branch-aware coverage. Every integration module retains at least 95% line coverage; setup/coordinators have 95% line and 91.67% branch coverage, and the new diagnostics and privacy-filter adapter paths have 100% line/branch coverage. Mypy is clean across 29 files, Ruff/Pylint pass, the network probe is blocked, and local layout/hassfest validation reports one valid integration. Matching HA beta 2026.8.0b2 and current beta b3 each passed the same 173-test suite; b3 remains informational because helper 0.13.350 pins b2. The runtime dependency closure audits cleanly. The complete stable graph still reports only the formally blocked Pillow/PyJWT findings already accepted by the owner and tracked in `TODO.md`; license inventory has no unknown result. The durable contract is in `docs/maintenance/COMPATIBILITY_SECURITY.md`; the handoff retains the point-in-time evidence.
...
1. Replace the preliminary CI with a clear workflow set. Recommended logical separation:
   ...
   - `version.yml`: a separate pull-request check that requires `.version` to increase relative to the PR base;
   - `release.yml`: trusted `master`-push quality gates followed by version/tag/release publication;
   - `dependency-review.yml`: dependency/security review where supported;
   - `compatibility.yml`: pull-request, `master` push, and manually triggered stable/beta and dependency-resolution checks.
...
6. Ensure untrusted pull-request code and content cannot access secrets or execute with write tokens.
7. Add action syntax/security validation, such as `actionlint` and a current GitHub Actions security analyzer where practical.
   Store every repository-authored CI helper as Python under `.github/scripts/`; do not add shell, JavaScript, or other helper-program files elsewhere. Inline workflow commands may invoke standard tools, and the API body supplied directly to the native `actions/github-script` action is the only JavaScript exception.
...
10. Run the bounded live FMI selection as a required step immediately after offline coverage in `ci.yml`, so pull requests and the reusable `master` release gate cannot pass without it. Run current stable and prerelease compatibility checks for pull requests, `master` pushes, and manual dispatch; keep prerelease informational because it is outside the supported release contract.
...
13. Validate release metadata:
   - use root `.version` as the only human-maintained version source and initialize it to `1.0.0`;
   - derive or verify every required version-bearing entity from `.version`, including the generated/mirrored `manifest.json` version, release tag, and release title;
   - require strict stable SemVer and a non-empty matching version section in `CHANGELOG.md`;
   - `hacs.json`;
...
15. Document the repository structure, HACS custom-repository discovery/install behavior, tag/release relationship, and owner release procedure under `docs/maintenance/`.
16. Enforce version progression twice:
   - a dedicated pull-request workflow must fail unless `.version` is greater than the target `master` version and all derived metadata/changelog content matches it;
   - the trusted `master` push workflow must fail before publication unless `.version` is greater than the pushed range's previous version.
17. After every successful, version-incrementing push to `master`, create an exact-version Git tag and a published GitHub Release for the pushed commit. Generate the release body from only the matching version section in `CHANGELOG.md` and append a link to the full changelog. The release job alone may receive `contents: write`; it must be concurrency-safe and idempotent for reruns. Use a full-SHA-pinned `actions/github-script` with its normal workflow token and prevalidated environment inputs for GitHub API access; do not implement GitHub API calls with `curl`, `gh api`, or a custom HTTP client in CI.
18. Do not manually publish a release, create a tag, push, or change remote repository settings in this session. Commit S15 and complete S16 on the same release branch with `.version` still at `1.0.0`; merging/pushing that completed release branch to `master` activates the checked-in publication automation only after the final documentation and verification are present.
19. Populate a managed block in each PR body from the first/latest `##` changelog section in the exact source-branch `CHANGELOG.md`; preserve text outside that block and refresh it on open, reopen, and source updates. Implement parsing only in a tested Python helper under `.github/scripts/`. To support forks, isolate the write in a `pull_request_target` workflow that checks out and executes only trusted default-branch code, obtains the head changelog through the native GitHub API client as inert data, receives no secrets, and has no repository-content write permission.
20. Remove every GitHub Actions `schedule` trigger. Delete the standalone `live-fmi.yml`; retain marker isolation but run its exact bounded command as the final test step in required `ci.yml`. Run compatibility for pull requests and `master` pushes as well as through `workflow_dispatch`; let CodeQL run for pull requests and `master` pushes and remain manually dispatchable.
21. Do not pass package names or versions directly to `pip install` in workflows, the Makefile, container recipes, or repository-authored CI helpers. Every pip installation must consume a repository or generated requirements/constraint file. Keep the supported development/test environment as a complete committed `pip freeze`, keep pip bootstrap itself in a dedicated exact requirements file, and isolate compatibility drift environments from that supported freeze.
22. Make compatibility jobs resolve the latest stable Home Assistant and latest available Home Assistant prerelease on every run without committed HA/helper/runtime version pins. Store unpinned direct resolution inputs in dedicated requirements files, immediately capture each selected transitive graph in a separate `requirements-compatibility-*-resolved.txt` with `pip freeze`, recreate a clean environment from that generated freeze, and run the suite only in the recreated environment. A beta/helper metadata mismatch may be informational only when it is the sole `pip check` conflict and the actual suite still passes.
23. Give every branch-safe standalone check the same event surface: CI, Home Assistant/HACS validation, CodeQL, compatibility, and version comparison run for pull requests to `master`, pushes to `master`, and manual dispatch from a selected branch. The version check compares manual runs with a selectable base ref defaulting to `master`, and compares `master` pushes with `github.event.before`. Keep reusable `workflow_call` entry points where release publication consumes them. Keep dependency review PR-only because its GitHub dependency diff is PR-specific, keep PR-body synchronization event-only, and keep release publication `master`-push-only. Because GitHub has no cross-workflow `needs`, the trusted release workflow must repeat its CI, validation, and version gates before publishing even though standalone `master` checks also run.
24. Inspect the first real PR workflow run with `gh`, wait for all jobs, and remediate every repository-controlled failure. Test first-release version comparison against the actual legacy root manifest layout, eliminate CodeQL findings rather than suppressing them, and account for GitHub's unsupported dependency-review API on fork repositories with an equivalent frozen-environment audit. Configure required HACS repository topics and Issues through the owner-authorized GitHub CLI session, then verify the rerun.
...
- No workflow contains a `schedule` trigger; compatibility runs for pull requests and `master`
  pushes and remains manually triggerable.
- HACS and hassfest validations pass.
- Installation/setup smoke test passes.
- Version tooling proves invalid, unchanged, decreased, mismatched-manifest, and missing-changelog cases fail, and produces the exact `1.0.0` release body/link in a dry run.
- PR-body tooling proves latest-section extraction, preservation/replacement semantics, malformed input rejection, and the GitHub body-size limit; actionlint and security review confirm that no source-branch code is checked out or executed with the PR write token.
- CI-policy tests prove the live marker runs after offline coverage on PR/release paths and that no standalone or scheduled live workflow remains.
- CI-policy tests prove pip installs are requirements-file based and compatibility jobs contain no hard-coded HA/helper release while freezing and recreating current stable/prerelease graphs before testing.
- CI-policy tests prove stable compatibility is a required event-driven signal, prerelease is
  informational, and every safe branch workflow has a valid manual-dispatch path without making
  publication or privileged PR metadata manually triggerable.
- CI-policy tests prove CI, validation, CodeQL, compatibility, and version checks all run as
  standalone workflows for PRs, `master` pushes, and manual dispatch, while the trusted release
  workflow independently repeats its publication gates.

S15 passed 210 stable offline tests with four live tests deselected and 96% aggregate branch-aware coverage, followed by a separate successful four-test live FMI run. Ruff formatting/linting, Pylint 10.00/10, mypy across 34 source files, hassfest, actionlint, layout/distribution setup, strict version metadata, the exact-exception dependency audit, and the deliberate network-blocking probe all pass. Sixteen release-contract tests include the actual legacy root-manifest first-release layout. Nine PR-body tests cover extraction and privileged-workflow security. Six CI-policy and four dependency-audit tests enforce trigger, install, dynamic compatibility, fork fallback, and vulnerability-exception contracts. Current-channel compatibility selected stable HA 2026.7.4/helper 0.13.348 and prerelease HA 2026.8.0b3/helper 0.13.351; both recreated freezes passed the then-current 204-test suite. The initial PR run exposed and led to fixes for the legacy manifest-path assumption, unsupported fork dependency-review API, obsolete coordinate-logging debug script, and missing HACS repository settings. At follow-up `f90707f`, all 12 current-head checks passed and PR #1 was `CLEAN`; S16 subsequently completed final local verification. Publication, branch rules, and the real HACS installation remain owner-controlled.
...
- All release metadata consistently targets version `1.0.0`.
- `.version` is the enforced source of truth, every PR/push requires an increment, and a successful `master` push publishes the matching tag/release only after its quality gates pass.
- PR bodies synchronize a bounded managed block from the exact source SHA's latest changelog section without executing source-branch code or overwriting manual context.
- No release or tag was published manually by the executing agent.
...
1. Rewrite `README.md` as a concise project entry point rather than a second user manual. It must contain:
   - a short but sufficient explanation of the project's purpose, intended use, FMI data source, and main forecast/observation/sensor capabilities;
   - the main user-visible limitations, including supported Home Assistant scope, dependence on external services, update behavior, manual location reconfiguration, and optional-feature constraints confirmed by the final implementation;
   - a structured documentation index linking the user guide, changelog, known limitations/TODOs, development and maintenance documentation, license, and other durable project documents;
   - the maintainer and canonical repository link.
   Move installation, upgrade, configuration, dashboard, troubleshooting, removal, privacy, and development details to their owning documents and remove their duplicated instructions from `README.md`. Keep only concise context and links needed to choose the correct document. Do not leave conflicting copies of user guidance in multiple places.
2. Create a separate concise user guide at `docs/USER_GUIDE.md` and link it prominently from `README.md`. It must focus on end users rather than contributors and include:
   - installing this fork through HACS as a custom repository;
   - manual installation and upgrade steps;
   - initial configuration, options, location reconfiguration, removal, and basic troubleshooting;
   - representative entity/device naming examples without assuming users retained generated entity IDs;
...
5. Finalize root `AGENTS.md`. It must be concise, accurate, and useful without rereading all historical handoffs. Include:
...
6. Keep `AGENTS.md` below Codex's project instruction size limit; link to detailed documents instead of copying them.
7. Resolve every remaining `xfail`, TODO, and tracker row associated with this plan. Any unrelated pre-existing TODO must be clearly distinguished.
8. Update all session rows, issue rows, and decision records.
9. Run final verification from a clean environment/checkout:
   - dependency sync;
   ...
   - vulnerability audit.
10. Review the complete diff from the original base for unrelated refactoring or accidental behavior changes.
11. Verify the repository is ready for owner-authorized release `1.0.0`, but do not tag, push, publish, or close issues without owner authorization.
12. Retain `.version` at `1.0.0` while S15 and S16 remain commits on the same release branch. The version-increment gate compares the completed branch with `master`; do not publish or merge the partial S15 state, and do not invent `1.0.1` merely for the S16 documentation commit.
...
- `docs/USER_GUIDE.md` gives a concise install/configuration path and usable typical-dashboard examples.
- Release notes, manifest, changelog, plan handoff, and manual steps consistently identify version `1.0.0`.
- The S16 handoff and release notes are complete, and maintenance documentation contains no plan-progress report.
...
| D021 | S13 | 2026-08-01 | Treat a finite set of FMI client/server, request transport, XML/parser, and external-shape exceptions as source-local availability failures at current, place, forecast, and station boundaries | Let DataUpdateCoordinator treat every non-client/server error as a whole-refresh failure; catch broad `Exception`; redesign or fork the client | Home Assistant regressions proved a request transport error skipped place fallback and a malformed forecast put setup into retry despite valid current weather. The finite tuple preserves source isolation and transition logging without a broad catch that would also absorb cancellation and unrelated exception classes. | `docs/maintenance/RUNTIME.md`; `tests/test_availability.py`; `c919057c6ddc39283bd88ba75bdf061b55ea9159` |
| D022 | S14 | 2026-08-01 | Store per-entry coordinators/listener in typed `ConfigEntry.runtime_data` and expose only explicitly sanitized health diagnostics | Retain the nested `hass.data` dictionary; omit diagnostics; include current weather/location in diagnostics | Current HA guidance treats runtime data as the typed config-entry lifecycle contract and explicitly identifies coordinates as sensitive diagnostics data. Lifecycle and sentinel-coordinate regressions prove the migration and redaction without changing persistent entry/registry identity. | `docs/maintenance/COMPATIBILITY_SECURITY.md`; `tests/test_compatibility_security.py`; `0143cd2f6741d6a55ce96b8710ad3eecadd82ce4` |
| D023 | S14 | 2026-08-01 | Support current HA stable only for the pre-1.0 maintenance line; run current beta informationally and make no previous-stable promise | Claim current and previous stable without a second lock; make beta blocking; replace stable with beta to obtain fixed transitive pins | The exact 2026.7.4 stable suite passes. Matching beta b2 and latest b3 pass informationally, but helper metadata is one beta behind; previous stable was not retained or tested. This keeps the user-facing support claim equal to reproduced evidence. | `docs/maintenance/COMPATIBILITY_SECURITY.md`; `0143cd2f6741d6a55ce96b8710ad3eecadd82ce4` |
| D024 | S14 | 2026-08-01 | Retain cached/rate-limited Nominatim enrichment only for the owner's small private test deployment and require a provider/removal decision before broader distribution | Remove address enrichment immediately; assume the public service scales with future users; send the configured coordinate | Only public FMI strike coordinates leave the FMI boundary, never the configured home coordinate. The current implementation meets the policy's 4/min periodic-script limit and attribution/User-Agent/cache expectations, but the policy discourages periodic application use at scale and expects switchability. | `TODO.md`; `docs/maintenance/COMPATIBILITY_SECURITY.md`; `0143cd2f6741d6a55ce96b8710ad3eecadd82ce4` |
| D025 | S15 | 2026-08-01 | Make root `.version` the only human-maintained project version and enforce a strict increase plus synchronized manifest and curated changelog section on every PR and `master` push | Maintain the manifest independently; derive from Git tags; permit unchanged-version maintenance commits | HACS reads the latest published Release tag while Home Assistant requires a committed manifest version. A small tested Python helper keeps those consumer-facing mirrors deterministic, rejects ambiguous SemVer, and handles the legacy pre-`.version` base without Git in the Python container. | `.github/scripts/version.py`; `tests/test_release_version.py`; `docs/maintenance/HACS_RELEASES.md`; `ff00b62eeb94875a7729e585a00e9c6f94da2027`; `f90707fc3efaa3063e922c9f64e9f671f002ab3c` |
| D026 | S15 | 2026-08-01 | Publish only after reusable CI/validation gates, with `contents: write` confined to a final SHA-pinned `actions/github-script` job; give branch-safe checks uniform PR/`master`/manual triggers; run bounded live FMI after offline coverage; keep prerelease compatibility informational; prohibit scheduled workflows | Publish from a custom HTTP script; trust independent master workflow results without an express dependency; grant workflow-wide content write; keep live/compatibility on cron; make live optional; use mutable action/container tags | GitHub Actions has no cross-workflow `needs`, so standalone master checks provide consistent visibility while the Release run deliberately repeats CI, validation, and version gates before publication. Required quality jobs remain read-only, the D027 metadata-only exception cannot write contents, immutable references reduce supply-chain drift, and only the final publish job can write contents. | `docs/maintenance/CI.md`; `.github/workflows/`; `ff00b62eeb94875a7729e585a00e9c6f94da2027`; `f90707fc3efaa3063e922c9f64e9f671f002ab3c` |
| D027 | S15 | 2026-08-01 | Use one metadata-only `pull_request_target` workflow to synchronize a marker-delimited PR-body block from the source changelog while executing only trusted default-branch code | Use ordinary `pull_request` and omit fork support; check out the head branch with a write token; overwrite the entire body; use a custom API client | Fork `pull_request` tokens cannot reliably write PR metadata. GitHub documents `pull_request_target` as suitable for authenticated PR automation when head code is not executed. The workflow reads only `CHANGELOG.md` at the exact head SHA through SHA-pinned `actions/github-script`, passes it as bounded inert data to a tested base-branch Python helper, preserves manual body text, refuses a concurrent-edit overwrite, has no secrets/content write/cache, and is covered by actionlint plus explicit security-contract tests. | `.github/workflows/pr-body.yml`; `.github/scripts/pr_body.py`; `tests/test_pr_body.py`; `docs/maintenance/CI.md`; `ff00b62eeb94875a7729e585a00e9c6f94da2027` |
...
- lightning maximum-age enhancement, wind-gust availability regression, sensor location-grouping regression, incorrect daily forecast regression, location reconfiguration requirement, and forecast-outage availability regression meet their required outcomes.
- polar sunrise/sunset regression's polar failure is fixed with tests.
...
- Event-driven and manually dispatchable jobs check current Home Assistant stable/prerelease drift, while every required PR and `master` CI run checks real FMI behavior.
- Workflows use least privilege and immutable action pins.
- Dependency update automation exists.
- README is a concise project overview and structured documentation index without duplicated user instructions; the linked user guide includes installation, configuration, and typical dashboard examples, and the changelog, migration notes, current maintenance contracts, and plan handoffs are current.
- Release metadata and documentation consistently target version `1.0.0`.
...
### Repository and known work

- Repository: <https://github.com/kogeler/fmi-hass-custom>
- Known work: Lightning maximum age; Wind gust unavailable; Sensors not grouped/named by location; Incorrect daily forecast; Location selection/editing; Forecast failure disables observations; Polar sunrise/sunset.
...
6. S16 user documentation, maintenance-document ownership map, recommended version, changelog, and release steps.
