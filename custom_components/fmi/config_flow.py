...
from . import base_unique_id, const
...
async def validate_user_config(hass: core.HomeAssistant, data):
    ...
    try:
        ...
        err_string = (
            "Client error with status " + str(err.status_code) + " and message " + err.message
        )
        errors = "client_connect_error"
        const.LOGGER.error(err_string)
    except fmi_erros.ServerError as err:
        err_string = "Server error with status " + str(err.status_code) + " and message " + err.body
    ...
    async def async_step_user(self, user_input=None):
        ...
        if user_input is not None:
            await self.async_set_unique_id(
                base_unique_id(
                    user_input[ha_const.CONF_LATITUDE], user_input[ha_const.CONF_LONGITUDE]
                )
            )
        ...
        data_schema = vol.Schema(
            {
                ...
                vol.Required(
                    ha_const.CONF_LATITUDE, default=self.hass.config.latitude
                ): cv.latitude,
                vol.Required(
                    ha_const.CONF_LONGITUDE, default=self.hass.config.longitude
                ): cv.longitude,
            }
        )
            ...
            data_schema=vol.Schema(
                {
                    vol.Optional(
                        ...
                        default=_options.get(const.CONF_FORECAST_DAYS, const.DAYS_DEFAULT),
                    ): vol.In(const.DAYS_RANGE),
                    vol.Optional(
                        ha_const.CONF_OFFSET,
                        default=_options.get(ha_const.CONF_OFFSET, const.FORECAST_OFFSET[0]),
                    ): vol.In(const.FORECAST_OFFSET),
                    vol.Optional(
                        const.CONF_MIN_HUMIDITY,
                        default=_options.get(const.CONF_MIN_HUMIDITY, const.HUMIDITY_MIN_DEFAULT),
                    ): vol.In(const.HUMIDITY_RANGE),
                    vol.Optional(
                        const.CONF_MAX_HUMIDITY,
                        default=_options.get(const.CONF_MAX_HUMIDITY, const.HUMIDITY_MAX_DEFAULT),
                    ): vol.In(const.HUMIDITY_RANGE),
                    vol.Optional(
                        const.CONF_MIN_TEMP,
                        default=_options.get(const.CONF_MIN_TEMP, const.TEMP_MIN_DEFAULT),
                    ): vol.In(const.TEMP_RANGE),
                    vol.Optional(
                        const.CONF_MAX_TEMP,
                        default=_options.get(const.CONF_MAX_TEMP, const.TEMP_MAX_DEFAULT),
                    ): vol.In(const.TEMP_RANGE),
                    vol.Optional(
                    ...
                    vol.Optional(
                        ...
                        default=_options.get(const.CONF_DAILY_MODE, const.DAILY_MODE_DEFAULT),
                    ): cv.boolean,
                    vol.Optional(
                        const.CONF_LIGHTNING,
                        default=_options.get(const.CONF_LIGHTNING, const.LIGHTNING_DEFAULT),
                    ): cv.boolean,
                    vol.Optional(
                    ...
                    vol.Optional(
                        ...
                        default=_options.get(const.CONF_OBSERVATION_STATION, 0),
                    ): cv.positive_int,
                }
            ),
        )
